...
import { ChevronRight, Heart, Droplet, Star, Plus, Sparkles, X, Check, Target } from "lucide-react";
...
export default function QuestsPage() {
  ...
  return (
    <div className="flex flex-col gap-5 p-5 max-w-md mx-auto pb-28">
      ...
      <button
        onClick={() => {
          setNewLevelNum((character?.level ?? 12) + 1);
          setShowLevelUp(true);
        }}
        className="mt-2 p-4 bg-yellow-400 dark:bg-amber-500 font-bold text-yellow-900 dark:text-amber-950 rounded-2xl w-full text-center shadow-sm hover:scale-[1.02] active:scale-[0.98] transition-transform"
      >
        Simulate Level Up
      </button>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-[18px] font-black text-slate-800">Add Daily Goal</h2>
                </div>
                <button
              ...
              <form onSubmit={handleCreateQuest} className="space-y-4">
                ...
                >
                  <Plus className="w-4 h-4 stroke-[3]" />
                  <span>{isSubmitting ? "Forging Quest..." : "Add to Daily Quests"}</span>
                </button>
              </form>
            </motion.div>
          </div>
...
function QuestCard({ icon, iconPath, title, desc, xp, checked, onToggle }: any) {
  return (
    <motion.div
      whileTap={{ scale: 0.98 }}
      onClick={onToggle}
      className={`bg-white dark:bg-[#1f1b4a] rounded-[28px] p-4 shadow-soft dark:shadow-none flex items-center gap-4 transition-opacity cursor-pointer border border-transparent dark:border-[#2e2959] ${
